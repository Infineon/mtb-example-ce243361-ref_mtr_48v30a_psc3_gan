...
#if (MOTOR_CTRL_MOTOR1_ENABLED)
static void MCU_RestartKilledPWMs_M1(void);
#endif
...
RAMFUNC_BEGIN
void MCU_DisUVWGateDriver()
{
#if defined(COMPONENT_CAT1)
    Cy_GPIO_Clr(EN_GD_PORT, EN_GD_NUM);
#elif defined(COMPONENT_CAT3)
    XMC_GPIO_SetOutputLow(EN_GD_PORT, EN_GD_PIN);
#endif
}
RAMFUNC_END

RAMFUNC_BEGIN
void MCU_EnUVWGateDriver()
{
#if defined(COMPONENT_CAT1)
    Cy_GPIO_Set(EN_GD_PORT, EN_GD_NUM);
#elif defined(COMPONENT_CAT3)
    XMC_GPIO_SetOutputHigh(EN_GD_PORT, EN_GD_PIN);
#endif
}
RAMFUNC_END

RAMFUNC_BEGIN
void MCU_TurnOff7thFET()
{
#if defined(COMPONENT_CAT1)
    Cy_GPIO_Clr(EN_SS_7FET_PORT, EN_SS_7FET_NUM);
#elif defined(COMPONENT_CAT3)
    XMC_GPIO_SetOutputLow(EN_SS_7FET_PORT, EN_SS_7FET_PIN);
#endif
}
RAMFUNC_END

RAMFUNC_BEGIN
void MCU_TurnOn7thFET()
{
#if defined(COMPONENT_CAT1)
    Cy_GPIO_Set(EN_SS_7FET_PORT, EN_SS_7FET_NUM);
#elif defined(COMPONENT_CAT3)
    XMC_GPIO_SetOutputHigh(EN_SS_7FET_PORT, EN_SS_7FET_PIN);
#endif
}
RAMFUNC_END
...
void MCU_RunISR0()
{
...
#endif
    Cy_GPIO_Write(SPI_CS_5012B_1_PORT, SPI_CS_5012B_1_PIN, 1UL);                // Set slave select pin manually by setting P8.2 to low
    ...
    //----------------------------------------------------------------------------------
    #if (USING_TLI_5012B)
    Get_Pos_TLI_5012B(&AppParams.TLI_5012B, 0, motor[0].params_ptr->motor.P);

    #if (OFFSET_CAL_DONE)
    motor[0].vars_ptr->w_fb.elec    = AppParams.TLI_5012B.PLL.Omega_flt;
    motor[0].vars_ptr->th_r_fb.elec = AppParams.TLI_5012B.PLL.Theta_flt;
    motor[0].vars_ptr->th_r_fb.mech = AppParams.TLI_5012B.Theta_MechRaw;
    #endif
    #endif
    //----------------------------------------------------------------------------------
    ...
    motor[0].sensor_iface_ptr->i_samp_0.raw = mcu[0].adc_scale.i_u * (Curr_ADC_Half_Point_Ticks - (uint16_t)mcu[0].dma_results[mcu[0].adc_mux.idx_isamp[0]])*motor[0].params_ptr->sys.analog.shunt.current_sense_polarity;
    motor[0].sensor_iface_ptr->i_samp_1.raw = mcu[0].adc_scale.i_v * (Curr_ADC_Half_Point_Ticks - (uint16_t)mcu[0].dma_results[mcu[0].adc_mux.idx_isamp[1]])*motor[0].params_ptr->sys.analog.shunt.current_sense_polarity;
    motor[0].sensor_iface_ptr->i_samp_2.raw = mcu[0].adc_scale.i_w * (Curr_ADC_Half_Point_Ticks - (uint16_t)mcu[0].dma_results[mcu[0].adc_mux.idx_isamp[2]])*motor[0].params_ptr->sys.analog.shunt.current_sense_polarity;   
#if defined(ANALOG_ROUTING_MUX_RUNTIME)
    if(mcu[0].adc_mux.en)
    {
    ...
    motor[0].sensor_iface_ptr->v_dc.raw = (mcu[0].adc_scale.v_dc * (uint16_t)mcu[0].dma_results[ADC_VBUS]); //Add 1.2V for 7th FET reverse voltage if powered up by VINVERTER
    ...
    // Reduce the duty cycle to ensure the safety, also delete small pulses    
    motor[0].vars_ptr->d_uvw_cmd.u = (motor[0].vars_ptr->d_uvw_cmd.u < 0.025f)? 0.00f : SAT(0.025f, 0.95f, motor[0].vars_ptr->d_uvw_cmd.u);
    motor[0].vars_ptr->d_uvw_cmd.v = (motor[0].vars_ptr->d_uvw_cmd.v < 0.025f)? 0.00f : SAT(0.025f, 0.95f, motor[0].vars_ptr->d_uvw_cmd.v);
    motor[0].vars_ptr->d_uvw_cmd.w = (motor[0].vars_ptr->d_uvw_cmd.w < 0.025f)? 0.00f : SAT(0.025f, 0.95f, motor[0].vars_ptr->d_uvw_cmd.w);
    // Reduce the duty cycle to ensure the safety, also delete small pulses    
    motor[0].vars_ptr->d_uvw_cmd_fall.u = (motor[0].vars_ptr->d_uvw_cmd_fall.u < 0.025f)? 0.00f : SAT(0.025f, 0.95f,motor[0].vars_ptr->d_uvw_cmd_fall.u);
    motor[0].vars_ptr->d_uvw_cmd_fall.v = (motor[0].vars_ptr->d_uvw_cmd_fall.v < 0.025f)? 0.00f : SAT(0.025f, 0.95f,motor[0].vars_ptr->d_uvw_cmd_fall.v);
    motor[0].vars_ptr->d_uvw_cmd_fall.w = (motor[0].vars_ptr->d_uvw_cmd_fall.w < 0.025f)? 0.00f : SAT(0.025f, 0.95f,motor[0].vars_ptr->d_uvw_cmd_fall.w);
...
#elif defined(COMPONENT_CAT3)
    XMC_CCU8_SLICE_SetTimerCompareMatchChannel1(PWM_U_HW, pwm_u_cc0);
    XMC_CCU8_SLICE_SetTimerCompareMatchChannel2(PWM_U_HW, pwm_u_cc1);
    XMC_CCU8_SLICE_SetTimerCompareMatchChannel1(PWM_V_HW, pwm_v_cc0);
    XMC_CCU8_SLICE_SetTimerCompareMatchChannel2(PWM_V_HW, pwm_v_cc1);
    XMC_CCU8_SLICE_SetTimerCompareMatchChannel1(PWM_W_HW, pwm_w_cc0);
    XMC_CCU8_SLICE_SetTimerCompareMatchChannel2(PWM_W_HW, pwm_w_cc1);
    ...
    #if defined(MOTOR_CTRL_ENABLE_DC_CHOPPER)
    DCChopper_Braking(&motor[0]);
    #endif
...
void MCU_InitADCs()
{
    ...
    //   mcu[0].adc_scale.i_uvw = (ADC_VREF_GAIN * CY_CFG_PWR_VDDA_MV * 1.0E-3f) / ((1<<12U) * motor[0].params_ptr->sys.analog.shunt.current_sensitivity * cs_gain); // [A/ticks]
      mcu[0].adc_scale.i_u = (ADC_VREF_GAIN * CY_CFG_PWR_VDDA_MV * 1.0E-3f) / ((1<<12U) * AppParams.cs.current_u_sensitivity * cs_gain); // [A/ticks]
      mcu[0].adc_scale.i_v = (ADC_VREF_GAIN * CY_CFG_PWR_VDDA_MV * 1.0E-3f) / ((1<<12U) * AppParams.cs.current_v_sensitivity * cs_gain); // [A/ticks]
      mcu[0].adc_scale.i_w = (ADC_VREF_GAIN * CY_CFG_PWR_VDDA_MV * 1.0E-3f) / ((1<<12U) * AppParams.cs.current_w_sensitivity * cs_gain); // [A/ticks]
      mcu[0].adc_scale.i_dc = (ADC_VREF_GAIN * CY_CFG_PWR_VDDA_MV * 1.0E-3f) / ((1<<12U) * AppParams.cs.current_dc_sensitivity * cs_gain); // [A/ticks]
    }
    else
    {
...
void MCU_RunISR0_M1()
{
    ...
    //----------------------------------------------------------------------------------
    #if (USING_TLI_5012B)
    Get_Pos_TLI_5012B(&AppParams.TLI_5012B_M1, 1, motor[1].params_ptr->motor.P);

    #if (OFFSET_CAL_DONE)
    motor[1].vars_ptr->w_fb.elec    = AppParams.TLI_5012B_M1.PLL.Omega_flt;
    motor[1].vars_ptr->th_r_fb.elec = AppParams.TLI_5012B_M1.PLL.Theta_flt;
    motor[1].vars_ptr->th_r_fb.mech = AppParams.TLI_5012B_M1.Theta_MechRaw;
    #endif
    #endif
    //----------------------------------------------------------------------------------
    ...
    uint32_t pwm_u_cc0 = (uint32_t)(mcu[1].pwm.duty_cycle_coeff * motor[1].vars_ptr->d_uvw_cmd.u);
    uint32_t pwm_v_cc0 = (uint32_t)(mcu[1].pwm.duty_cycle_coeff * motor[1].vars_ptr->d_uvw_cmd.v);
    uint32_t pwm_w_cc0 = (uint32_t)(mcu[1].pwm.duty_cycle_coeff * motor[1].vars_ptr->d_uvw_cmd.w);

    uint32_t pwm_u_cc1 = (uint32_t)(mcu[1].pwm.duty_cycle_coeff * motor[1].vars_ptr->d_uvw_cmd_fall.u);
    uint32_t pwm_v_cc1 = (uint32_t)(mcu[1].pwm.duty_cycle_coeff * motor[1].vars_ptr->d_uvw_cmd_fall.v);
    uint32_t pwm_w_cc1 = (uint32_t)(mcu[1].pwm.duty_cycle_coeff * motor[1].vars_ptr->d_uvw_cmd_fall.w);

    Cy_TCPWM_PWM_SetCompare0BufVal(PWM_U_M1_HW, PWM_U_M1_NUM, pwm_u_cc0);
    Cy_TCPWM_PWM_SetCompare1BufVal(PWM_U_M1_HW, PWM_U_M1_NUM, pwm_u_cc1);
    Cy_TCPWM_PWM_SetCompare0BufVal(PWM_V_M1_HW, PWM_V_M1_NUM, pwm_v_cc0);
    Cy_TCPWM_PWM_SetCompare1BufVal(PWM_V_M1_HW, PWM_V_M1_NUM, pwm_v_cc1);
    Cy_TCPWM_PWM_SetCompare0BufVal(PWM_W_M1_HW, PWM_W_M1_NUM, pwm_w_cc0);
    Cy_TCPWM_PWM_SetCompare1BufVal(PWM_W_M1_HW, PWM_W_M1_NUM, pwm_w_cc1);
...
//    // Detect kill via TCPWM: counter not running indicates kill event
//    if (!(Cy_TCPWM_PWM_GetStatus(PWM_U_M1_HW, PWM_U_M1_NUM) & CY_TCPWM_PWM_STATUS_COUNTER_RUNNING))
//    {
//        motor[1].sensor_iface_ptr->digital.fault = true;
//    }
...
#endif
#if (DRIVE_ENABLE_CONTROL_POT)
    DriveEnableControlforPot(&motor[1]);  // Update drive enable/disable based on potentiometer command (Motor 1)
#endif
...
void MCU_InitADCs_M1()
{
    ...
    mcu[1].adc_scale.v_uvw = (ADC_VREF_GAIN * CY_CFG_PWR_VDDA_MV * 1.0E-3f) / ((1<<12U) * motor[1].params_ptr->sys.analog.volt.vuvw_adc_scale); // [V/ticks]
    mcu[1].adc_scale.v_dc = (ADC_VREF_GAIN * CY_CFG_PWR_VDDA_MV * 1.0E-3f) / ((1<<12U) * motor[1].params_ptr->sys.analog.volt.vdc_adc_scale); // [V/ticks]
...
void MCU_InitTimers_M1()
{
       // Clock frequencies .......................................................
        ...
        mcu[1].pwm.deadtime = ((uint32_t)(mcu[1].clk.tcpwm * motor[1].params_ptr->sys.samp.deadtime));
        mcu[1].isr0.count = 0U;
        ...
        Cy_TCPWM_PWM_PWMDeadTime(PWM_U_M1_HW, PWM_U_M1_NUM, mcu[1].pwm.deadtime); // set dead time value
        Cy_TCPWM_PWM_PWMDeadTimeN(PWM_U_M1_HW, PWM_U_M1_NUM, mcu[1].pwm.deadtime); // set dead time value
        Cy_TCPWM_PWM_PWMDeadTimeBuff(PWM_U_M1_HW, PWM_U_M1_NUM, mcu[1].pwm.deadtime); // set dead time value
        Cy_TCPWM_PWM_PWMDeadTimeBuffN(PWM_U_M1_HW, PWM_U_M1_NUM, mcu[1].pwm.deadtime); // set dead time value
        ...
        Cy_TCPWM_PWM_PWMDeadTime(PWM_V_M1_HW, PWM_V_M1_NUM, mcu[1].pwm.deadtime); // set dead time value
        Cy_TCPWM_PWM_PWMDeadTimeN(PWM_V_M1_HW, PWM_V_M1_NUM, mcu[1].pwm.deadtime); // set dead time value
        Cy_TCPWM_PWM_PWMDeadTimeBuff(PWM_V_M1_HW, PWM_V_M1_NUM, mcu[1].pwm.deadtime); // set dead time value
        Cy_TCPWM_PWM_PWMDeadTimeBuffN(PWM_V_M1_HW, PWM_V_M1_NUM, mcu[1].pwm.deadtime); // set dead time value
        ...
        Cy_TCPWM_PWM_PWMDeadTime(PWM_W_M1_HW, PWM_W_M1_NUM, mcu[1].pwm.deadtime); // set dead time value
        Cy_TCPWM_PWM_PWMDeadTimeN(PWM_W_M1_HW, PWM_W_M1_NUM, mcu[1].pwm.deadtime); // set dead time value
        Cy_TCPWM_PWM_PWMDeadTimeBuff(PWM_W_M1_HW, PWM_W_M1_NUM, mcu[1].pwm.deadtime); // set dead time value
        Cy_TCPWM_PWM_PWMDeadTimeBuffN(PWM_W_M1_HW, PWM_W_M1_NUM, mcu[1].pwm.deadtime); // set dead time value
