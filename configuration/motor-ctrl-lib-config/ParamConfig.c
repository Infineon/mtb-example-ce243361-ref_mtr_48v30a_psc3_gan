...
void APP_PARAMS_InitManual(APPPARAMS_t* app_params_t);
...
#if (MOTOR_CTRL_MOTOR1_ENABLED)
STATIC_ASSERT(!((ADC_CS_SHUNT_TYPE_M1 == Single_Shunt) && (MOTOR_CTRL_VOLT_MOD_SCHEME_M1 == Neutral_Point_Modulation) &&(MOTOR_CTRL_SS_MEAS_TYPE_M1 == Phase_Shift)),
              "Single_Shunt - Phase shift PWM with Neutral_Point_Modulation is not supported");
#endif
...
void PARAMS_Init(MOTOR_t *motor_ptr)
{
    ...
    if (!params_id_check || PARAMS_ALWAYS_OVERWRITE)
    {
        if(motor_ptr->motor_instance  ==0)
        {
            ...
            APP_PARAMS_InitManual(&AppParams);
...
void APP_PARAMS_InitManual(APPPARAMS_t* app_params_t)
{
    AppParams.cs.current_u_sensitivity = ADC_CS_U_CURRENT_SENSITIVITY;
    AppParams.cs.current_v_sensitivity = ADC_CS_V_CURRENT_SENSITIVITY;    
    AppParams.cs.current_w_sensitivity = ADC_CS_W_CURRENT_SENSITIVITY;
    AppParams.cs.current_dc_sensitivity = ADC_CS_DC_CURRENT_SENSITIVITY;

    AppParams.TLI_5012B.Theta_MechRaw_U16    = 0;           // encoder position data (mechanical angle)
    AppParams.TLI_5012B.Theta_MechRaw         = 0;          // -pi ~ pi
    AppParams.TLI_5012B.Theta_TLI_5012B_U16  = 0;           //
    AppParams.TLI_5012B.TLI5012B_Offset_S16 = TLI5012B_OFFSET_S16;      // [cnt] TLI-5012 Position Sensor Offset: -32768 ~ 32767[cnt] corresponding to -180.0 ~ 180.0[deg]
    AppParams.TLI_5012B.Dir_TLI_5012B        = 1;           //
    AppParams.TLI_5012B.Theta_TLI_5012B_flt  = 0.0f;        //
    AppParams.TLI_5012B.PLLSmplCnt = 0;
    AppParams.TLI_5012B.MovingInitialized = 0;

    AppParams.TLI_5012B_M1.Theta_MechRaw_U16    = 0;        // encoder position data (mechanical angle)
    AppParams.TLI_5012B_M1.Theta_MechRaw          = 0;      // -pi ~ pi
    AppParams.TLI_5012B_M1.Theta_TLI_5012B_U16  = 0;        //
    AppParams.TLI_5012B_M1.TLI5012B_Offset_S16 = TLI5012B_OFFSET_S16_M1;        // [cnt] TLI-5012 Position Sensor Offset: -32768 ~ 32767[cnt] corresponding to -180.0 ~ 180.0[deg]
    AppParams.TLI_5012B_M1.Dir_TLI_5012B          = 1;      //
    AppParams.TLI_5012B_M1.Theta_TLI_5012B_flt  = 0.0f;     //
    AppParams.TLI_5012B_M1.PLLSmplCnt = 0;
    AppParams.TLI_5012B_M1.MovingInitialized = 0;

    AppParams.DCChopper.vdc_chopper_upper_thresh = MOTOR_CTRL_VDC_CHOPPER_UPPER;
    AppParams.DCChopper.vdc_chopper_lower_thresh = MOTOR_CTRL_VDC_CHOPPER_LOWER;
    AppParams.DCChopper.BrakeUsageRate = BRAKE_USAGE_RATE;

    AppParams.Cal_Speed_Method = CAL_SPD_METHOD;
}
...
void PARAMS_InitManual(PARAMS_t* params_ptr)
{
    ...
    params_ptr->sys.analog.offset_null_time = 0.030f; // [sec], this will determine the loop-gain of auto-offset-nulling loop
...
void PARAMS_InitManual_M1(PARAMS_t* params_ptr)
{
    ...
    params_ptr->sys.samp.deadtime = MOTOR_CTRL_PWM_DEADTIME_M1; // [sec]
    // Calibration Parameters:
    params_ptr->sys.analog.calib.i_u = (CALIB_PARAMS_t){ 1.0f, 0.0f }; // {%, A}
    params_ptr->sys.analog.calib.i_v = (CALIB_PARAMS_t){ 1.0f, 0.0f }; // {%, A}
    params_ptr->sys.analog.calib.i_w = (CALIB_PARAMS_t){ 1.0f, 0.0f }; // {%, A}
    ...
    params_ptr->sys.analog.filt.w0_pot = TAU_TO_RADSEC(0.200f); // [Ra/sec]
    // Shunt Parameters:
    params_ptr->sys.analog.shunt.type = ADC_CS_SHUNT_TYPE_M1;
    params_ptr->sys.analog.shunt.opamp_gain = ADC_CS_OPAMP_GAIN_M1;
    
    params_ptr->sys.analog.shunt.single_shunt.type = MOTOR_CTRL_SS_MEAS_TYPE_M1;
    params_ptr->sys.analog.shunt.single_shunt.adc_t_min = ADC_CS_SS_MIN_SEGMENT_TIME_M1; // [sec]
    params_ptr->sys.analog.shunt.single_shunt.ki = MOTOR_CTRL_SS_HMOD_KI_M1; // [#]
    params_ptr->sys.analog.shunt.single_shunt.adc_t_sh_delay =MOTOR_CTRL_SS_PS_SH_DELAY_M1; //[sec]
    ...
    params_ptr->sys.analog.volt.vdc_adc_scale = ADC_SCALE_VDC_M1; //[V/V]
    params_ptr->sys.analog.volt.vuvw_adc_scale = ADC_SCALE_VUVW_M1; //[V/V]
    ...
    params_ptr->sys.rate_lim.w_ol_cmd.elec = MECH_TO_ELEC(HZ_TO_RADSEC(RPM_TO_HZ(MOTOR_CTRL_SPEED_CMD_RATE_OPEN_LOOP_M1)), MOTOR_POLE_M1); // [(Ra/sec-elec)/sec]
#if defined(CTRL_METHOD_RFO) || defined(CTRL_METHOD_TBC)
...
#endif
#if defined(CTRL_METHOD_RFO)
    params_ptr->sys.rate_lim.p_cmd =  DEG_TO_RAD(MOTOR_CTRL_POSITION_CMD_RATE_M1);  //[Ra/sec] 
#endif
    // Fault Parameters:
    ...
    params_ptr->sys.faults.clr_try_period = MOTOR_CTRL_FAULT_CLR_TRY_PERIOD_M1;// [sec]    
    params_ptr->sys.faults.cmd_clr_thresh = 1; //[%] 
    
    params_ptr->sys.faults.watchdog_time = 1000; // [ms]
    
    params_ptr->sys.faults.phase_loss.tau = MOTOR_CTLR_FAULT_PHASE_LOSS_TIME_M1;//[sec]   
    params_ptr->sys.faults.phase_loss.zero_current_thres = MOTOR_CTLR_FAULT_PHASE_LOSS_MIN_CURRENT_M1; //[A]
    ...
    params_ptr->sys.boot_mode  = MOTOR_CTRL_BOOTSTRAP_MODE_M1;
#if defined(CTRL_METHOD_RFO) || defined(CTRL_METHOD_SFO)
    params_ptr->sys.dyno_lock_time = 0.500f; // [sec]
#endif
#if defined(CTRL_METHOD_RFO)
   params_ptr->sys.catch_spin.mode = MOTOR_CTRL_CATCH_SPIN_MODE_M1;
   params_ptr->sys.catch_spin.time = MOTOR_CTRL_CATCH_SPIN_TIME_M1; // [sec]
   params_ptr->sys.catch_spin.w_thresh.elec = MECH_TO_ELEC(HZ_TO_RADSEC(RPM_TO_HZ(MOTOR_CTRL_CATCH_SPIN_SPEED_THRESH_M1)), MOTOR_POLE_M1) ; // [Ra/sec-elec]
#endif
    ...
    params_ptr->obs.biquad_b[2] = 1.0f / HZ_TO_RADSEC(25.0f) / HZ_TO_RADSEC(75.0f); // [1/(Ra/sec)^2]
    params_ptr->obs.pll.w0 = HZ_TO_RADSEC(75.0f); // [Ra/sec]
    params_ptr->obs.pll.w_max.elec = MECH_TO_ELEC(HZ_TO_RADSEC(RPM_TO_HZ(MOTOR_CTRL_OBS_MAX_SPEED_M1)), MOTOR_POLE_M1);; // [Ra/sec-elec]
...
#endif
#if defined(CTRL_METHOD_RFO)
    params_ptr->ctrl.position.bw =HZ_TO_RADSEC(MOTOR_CTRL_POSITION_BW_M1);  //[Ra/sec]
...
void PARAMS_InitAutoCalc(PARAMS_t* params_ptr)
{
    ...
    {
      params_ptr->sys.faults.vdc_thresh.min = params_ptr->sys.vdc_nom * 0.6f; // [V]
      params_ptr->sys.faults.vdc_thresh.max = params_ptr->sys.vdc_nom * 1.1f; // [V]
    }
    ...
    // l:        10u -> 700u   ,[H]
    // f (=r/l): 500 -> 1000   ,[Hz]
    const MINMAX_t Range_R = { 3.0E-3f, 70.0E-3f }; // [Ohm]
